...
class LoggingAspect(
    ...
    @Value("\${skedulo.app.config.fetch-timeout-seconds}")
    private val fetchTimeoutSeconds: Int,
) {
...
//            val timeout = methodSignature.method.getAnnotation(WithTimeout::class.java)
//            if (timeout != null) {
//                val timeoutMillis = getTimeoutMillis(timeout.timeout, timeout.unit)
//                log.info { timeoutMillis }
//            }

            val timeoutMillis = getTimeoutMillis(coroutineLogExecutionTime)

            // Measure method execution time
            val stopWatch = StopWatch("$className -> $methodName")
            stopWatch.start(methodName)
            kotlin.runCatching {
                val result = when (timeoutMillis) {
                    0L -> proceedingJoinPoint.proceedCoroutine()
    ...
    private fun getTimeoutMillis(timeout: CoroutineLogExecutionTime): Long {
        if (!timeout.withTimeout) return 0L
